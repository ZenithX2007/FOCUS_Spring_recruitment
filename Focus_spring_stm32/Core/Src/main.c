...
void Command_Parse(char *cmd)
{
    ...
    switch (event)
    {
        ...
        case EVT_OFF:current_state=STOP;TT_SetSpeed(0);break;
        ...
        case EVT_SPEED:if (current_state == STOP || current_state == ERROR_STATE) {return;}
				               if (value < 0 || value > 150) {return;}
             
					            current_state=SET_SPEED;
